...
`brushcodex-standard` owns five package candidates (all `0.9.0-draft`, `private`, `type: module`):
...
5. Runs, under **plain Node ESM** and via the installed **`.bin` shims** from a foreign working
   ...
   subpath); fixtures enumerate the manifest and load the corpus (exactly **92**); the CLI reports
   **92/92** conformance and correct validate exit codes.
